#!/usr/bin/env node
/**
 * Листинги Mushroom Checker (локаль en-US) в App Store Connect и Google Play из
 * файлов `fastlane/metadata/checker/**` — чтобы тексты жили в репозитории, а не
 * только в веб-консолях.
 *
 ...
 * оговорки в текстах, а листинг соседа не должен зависеть от правки в чужом
 * флейворе (см. .cursor/rules/flavors.mdc).
 *
 * Аутентификация как в соседних скриптах: для ASC — JWT ES256 с
 * `aud: appstoreconnect-v1`, для Play — сервисный аккаунт из
...
const PKG = "ai.skyforest.mushroomchecker";
const LOCALE = "en-US";
...
for (const [key, { value, limit }] of Object.entries(TEXTS)) {
  ...
  if (tooLong) bad = true;
  console.log(`${tooLong ? "ДЛИННО" : "ok"}  ${key}: ${len} / ${limit}`);
...
try {
  ...
  if (APPLY) {
    ...
    await play("PUT", `/edits/${edit.id}/details`, {
      ...
      contactWebsite: "https://checker.skyforest.ai",
    });
...
try {
  ...
  const icon = await play("GET", `/edits/${check.id}/listings/${LOCALE}/icon`);
  const icons = (icon.images || []).length;
  if (icons !== 1) mismatch = true;
  console.log(`${icons === 1 ? "OK  " : "НЕТ "} Play icon: ${icons} шт.`);
