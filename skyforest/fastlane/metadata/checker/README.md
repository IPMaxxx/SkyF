...
**Цены в текстах однажды отстали от этой модели.** До августа 2026 описания в
`en-US/` и `android/en-US/`, промо-текст, заметки релиза и заметки ревьюеру
обещали «USD 2.00 per month or USD 14.99 per year» — месячный тариф к тому
времени был закрыт для новых покупок, а годовой стоил 39.99. Карточка предлагала
купить то, чего покупатель купить не мог. Сейчас во всех этих файлах стоит
«USD 5.00 per week or USD 39.99 per year», и тот же текст лежит в обоих сторах.

Заодно выяснилось, что расходиться тексты умеют в обе стороны: в консолях был
блок «YOUR FINDS» про историю и квесты, которого в репозитории не было вовсе.
Заливка из файлов стёрла бы его молча, поэтому **перед `--apply` стоит сверять
текст стора с файлом**, а не только длину: `node fastlane/checker-listings.mjs`
показывает длины, полный текст отдаёт `node fastlane/asc.mjs GET
"/v1/appStoreVersions/<id>/appStoreVersionLocalizations"`.

Повторения сторожит `node fastlane/.store-prices-check.mjs`: он читает цены
оттуда же, откуда их берёт пейволл (`FLAVORS.<id>.subscriptionPlan`), и падает,
если в текстах карточки названа сумма, которой нет среди продающихся товаров,
или названа при чужом расчётном периоде. Проверка офлайновая и идёт по всем трём
приложениям и всем языкам их карточек. Соседний `node
fastlane/.play-subs-check.mjs` сверяет с консолью сами товары; вместе они
закрывают и цену товара, и обещание о ней.

**Про состояние версии в App Store.** Список «редактируемых» состояний обманчив:
у версии в `WAITING_FOR_REVIEW` Apple принимает и `description`, и
`promotionalText` (правка уезжает в то же ревью), а закрыт там только
`whatsNew` — 409 `STATE_ERROR`. Поэтому `checker-listings.mjs` не гадает по
состоянию, а пробует записать и разбирает отказ; если App Store всё же закрыт
(так бывает у выпущенной версии), прогон не обрывается и Google Play заливается
всё равно. Флаг `--play-only` пропускает App Store принудительно.
