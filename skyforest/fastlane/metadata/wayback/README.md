...
2. **Дождаться одобрения недельного товара.** Пока он не `APPROVED`, App Store
   ...
   сейчас.
3. **Скрипт `fastlane/play-subs-create.mjs` устарел и опасен.** В его списке
   годовой Checker записан за 14.99 USD, тогда как в консоли 39.99, а WayBack —
   за 3.99 вместо 19.99. Прогон откатит цены обоих приложений. Товары WayBack
   заводятся из `fastlane/wayback-play-subs.mjs`; общий скрипт либо привести в
   порядок, либо разобрать на пофлейворные.
