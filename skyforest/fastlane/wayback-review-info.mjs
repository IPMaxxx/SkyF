#!/usr/bin/env node
/**
 * App Review Information версии WayBack в App Store Connect: демо-аккаунт,
 * контакт и заметки ревьюеру (`appStoreReviewDetails`).
 *
 * Зачем это обязательно именно у WayBack: приложение открывается гейтом, и без
 * активной подписки или пробного периода внутрь не попасть. Ревьюеру нужен
 * рабочий вход и явный порядок действий, иначе отказ по 2.1 практически
 * гарантирован — он просто не увидит приложение.
 *
 * Текст заметок — `fastlane/metadata/wayback/review-notes.txt`, почта и пароль
 * подставляются в него из констант ниже (или из переменных окружения
 * `WB_REVIEW_EMAIL` / `WB_REVIEW_PASSWORD`), чтобы пароль лежал в одном месте.
 *
 * Запуск из каталога skyforest:
 *   node fastlane/wayback-review-info.mjs           — показать, что сейчас в ASC
 *   node fastlane/wayback-review-info.mjs --apply   — записать и перечитать
 */
...
const DEMO_PASSWORD =
  process.env.WB_REVIEW_PASSWORD || "Sky#WswlragbAJk9OxYoofcR6kU9";
...
const NOTES = readFileSync(new URL("./metadata/wayback/review-notes.txt", HERE), "utf8")
  .trim()
