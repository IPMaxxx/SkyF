...
import { CkStatusCard } from "@/components/checker/primitives";
...
export function CheckerSocialButtons({ redirect }: { redirect: string }) {
...
export function CheckerAuthDivider() {
  const t = useTranslations("checker.auth");
  return (
    <div className="flex items-center gap-3">
      <i className="h-px flex-1 bg-ck-border-4" />
      <span className="ck-mono text-[10px] tracking-[0.12em] text-ck-muted-2">
        {t("orWithEmail")}
      </span>
      <i className="h-px flex-1 bg-ck-border-4" />
    </div>
