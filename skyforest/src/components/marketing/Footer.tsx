...
const SOCIAL_LINKS = BRAND.social.map((s) => ({
  label: s.label,
  href: s.href,
  svg: SOCIAL_ICONS[s.label] ?? null,
}));
        ...
        <div className="mt-10 grid gap-8 border-t border-white/10 pt-8 lg:grid-cols-[1.5fr_1fr]">
          <div>
            ...
            <ul className="space-y-1.5 text-sm text-white/60">
              <li className="text-white/80">{BRAND.company.legalName}</li>
              {BRAND.company.registrationLines.map((line) => (
                <li key={line}>{line}</li>
              ))}
              {BRAND.company.schedule && <li>{BRAND.company.schedule}</li>}
            </ul>
            <div className="mt-4">
