 ...
 * приложению). Приложение при этом задумано для леса без связи, поэтому
 ...
 * приложением; это осознанный обмен, и он дешевле, чем запертый в лесу
 * подписчик.
 *
...
export function useWaybackGate(enabled: boolean): WaybackGateState {
  ...
  const check = useCallback(async () => {
    ...
    // getSession() читает локальное хранилище и не ходит в сеть — единственный
    // способ узнать «вошёл ли» без связи. getUser() здесь не годится: без сети
    // он вернул бы ошибку, и гейт принял бы подписчика за анонима.
    ...
    try {
      ...
      // Блокировка хранилища auth-js. Сессию не прочитать — решаем по кешу.
      if (cached) {
        commit("allowed", true);
        return;
      }
