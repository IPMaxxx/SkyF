...
export async function GET(_request: Request, { params }: Ctx) {
  ...
  const { data: tour, error: tourErr } = await supabase
    .from("mushroom_tours")
    .select(
      "id, title, description, departure_lat, departure_lng, departure_desc, mushroom_species, tour_date, departure_time, spots, auction_start_at, auction_end_at, start_price, bid_step, currency, anti_snipe_seconds, confirm_window_hours, status, finished_at, created_at"
    )
    .eq("id", id)
