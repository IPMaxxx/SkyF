...
> **Документ снят до изоляции Checker в дерево `/ck/*` и до редизайна:** модель
> подписки в тексте актуальна, а описания маршрутов и экранов местами отстали —
> публичные URL те же, но страницы живут в `src/app/[locale]/ck/**` и компонентах
> `src/components/checker/**`. Нужна отдельная сверка со скриншотами.
...
- **Цель продукта:** платная подписка **Mushroom Checker Premium** без лимита
  распознаваний ($2/мес, $14.99/год) с трёхдневным триалом, в котором доступно
  10 распознаваний; покупка только через App Store / Google Play. Параметры
  модели — `FLAVORS.checker.subscriptionPlan` (`src/flavors/checker/config.ts`).
...
- **Элементы:** иконка `Crown`, заголовок **Premium subscription**, подзаголовок;
  ...
  карточка тарифа с бейджем **3-day trial**, ценой из стора, тремя фичами
  (`checkerF1..F3`), строкой «3 days free, then auto-renews. Cancel anytime.»,
...
8. Лимит триала (10 распознаваний за все три дня) проверяется на сервере; в
   оплаченной подписке лимита нет. При исчерпании — 402 и экран
   `identify.limitTitle` / `limitBody`, без подписки — `noSubTitle` / `noSubBody`.

### 4.3 Оформление подписки с трёхдневным триалом
...
4. **Subscribe to Mushroom Checker Premium** → системный диалог стора;
   первые 3 дня бесплатно, далее автопродление.
