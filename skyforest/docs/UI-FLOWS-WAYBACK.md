...
геолокация подменена на лес под Минском 53.9412 / 27.3168, боевой хост
...
**WayBack** — «не потеряться в лесу»: приложение запоминает точку входа в лес и
...
- **Аудитория:** грибники, ягодники, любители прогулок по лесу — люди без
  туристических навигаторов, у которых в лесу пропадает сеть.
...
- **Элементы:** логотип `public/images/logo-square.png`, тег-лайн
  `common.splashTagline`, три пульсирующие точки; фон — радиальный градиент `#0e1710`.
- **Тайминги:** 1300 мс показа + 500 мс затухания, страховка 3000 мс.
- ⚠️ **Находка:** как и в Checker, используется **общий логотип SkyForest**, а не
  иконка WayBack — приложение открывается «чужим» брендом.
...
- **Элементы:** **Continue with Google** / **Continue with Apple**
  (`src/components/auth/SocialLoginButtons.tsx`), разделитель `OR WITH EMAIL`,
  поля Email / Password, **Forgot password?**, кнопка **Sign in**,
...
- ⚠️ **Находка:** иконка на экране входа и тег-лайн — общие для SkyForest
  (`auth.nativeSlogan` про грибные прогнозы), к WayBack они не относятся.
...
- **Элементы:**
  ...
  - главная кнопка **I'm entering the forest** (`btn-primary`, иконка `Footprints`,
...
- ⚠️ **Находка:** на скриншоте карта открыта на весь мир — без GPS-фикса нет
  разумного центра. При редизайне нужен фолбэк-центр (последняя известная позиция
  или центр скачанного офлайн-региона).
...
- **Элементы (сверху вниз):**
  - две плитки: **To entry point** (расстояние, крупным акцентным цветом) и
    **Time in forest** (`H:MM` + строка `since 22:00`);
  ...
  - легенда: **Dashed line — stretches with no recording (the app was in the background).**;
  - блок **Offline map** (тот же менеджер, центр — якорь похода);
  - кнопка **I'm out of the forest** (красная, вторичная).
...
- **Данные:** точки пишет глобальный `TrackRecorder` из `(app)`-layout
  (`src/lib/trackRecorder.ts`): непрерывный `watchPosition`, пока приложение
  активно, плюс внеочередной замер при возврате из фона и при открытии страницы;
  страница подписана на событие `sf:track-capture`. Активный поход хранится в
  `localStorage["sf_active_track"]` и зеркалируется в Capacitor Preferences —
  именно оттуда его читает автономный офлайн-экран.
- **Тикер:** строка «в пути» обновляется раз в 30 с.

### 3.13 Подтверждение выхода из леса
...
- **Пустое состояние отсутствует:** при пустом списке компонент возвращает `null`,
  и блока в интерфейсе просто нет.
...
- **Карточки:** **Profile** (email, имя — `EditProfileName.tsx`, контакт —
  `EditContactLink.tsx`), **Change password** (`ChangePassword.tsx`),
  **Two-factor authentication** (`TwoFactorSetup.tsx`), **App Lock**
  (`src/components/native/BiometricLockSetting.tsx` — только в нативе и только
  при наличии биометрии; флаг в Preferences `biometric_lock_enabled`),
  история транзакций (`TransactionHistory.tsx`), **Delete account**
  (`DeleteAccount.tsx`).
- **Данные:** Supabase `profiles`, `token_balances`, `token_transactions`;
  без сессии — `redirect("/login")`.
- ⚠️ **Находка:** экран целиком унаследован от SkyForest — в WayBack не нужны
  ни баланс токенов, ни история транзакций; подзаголовок про токены вводит в
  заблуждение. Здесь же логично показывать статус подписки.
...
холодном старте). Это единственный экран, который гарантированно работает в лесу
...
- **Элементы:** плитки **До входа** (расстояние) и **В лесу** (время), круглый
  ...
  нижние кнопки **Я вышел из леса** (красная) и **Открыть приложение**.
...
1. `/dashboard/track` → **I'm entering the forest**.
...
5. Пока приложение открыто, `TrackRecorder` пишет точки (`watchPosition`);
   при возврате из фона делается внеочередной замер. Промежутки без записи
   показываются на карте **пунктиром**.
...
7. **I'm out of the forest** → карточка подтверждения → **Yes, finish**.
...
- **Офлайн-экран оболочки — отдельная кодовая база.** Vanilla JS + инлайн-CSS,
  ...
  приложение в лесу будет выглядеть иначе, чем в городе.
...
- **Safe-area и тач-таргеты.** `pt-[env(safe-area-inset-top)]`,
  ...
  Главная кнопка старта должна оставаться крупной: её нажимают в лесу, в перчатках.
...
1. Splash и экран входа используют брендинг SkyForest (общий логотип, тег-лайн
   про грибные прогнозы) — к WayBack не относится.
2. Ручной выбор точки входа без GPS-фикса открывает карту мира — нужен разумный
   центр по умолчанию.
3. `/account` унаследован целиком от SkyForest: баланс токенов и история
   транзакций в WayBack не нужны; статуса подписки на экране нет.
4. У истории походов нет пустого состояния — блок просто исчезает.
5. Из анонимного режима нет явного приглашения войти/оформить подписку.
