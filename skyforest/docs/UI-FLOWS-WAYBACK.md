...
- **Элементы (сверху вниз):**
  ...
  - легенда пунктира — **две строки, выбор по возможностям текущей сборки**
    (см. «Легенда пунктира и обещание про фон» ниже): без нативной фоновой
    службы **dashed — stretches with no recording** (`gapHint`), с ней
    **dashed — stretches where the phone could not see satellites**
    (`gapHintBackground`);
...
#### Легенда пунктира и обещание про фон

Веб приезжает в оболочку с сайта, то есть один и тот же текст одновременно
читают во всех установленных приложениях — включая сборку из Play без фоновой
записи и ту, что лежит на ревью в магазине. Поэтому строк о записи по две, и
выбор между ними делается в рантайме:

| ключ | когда показывается |
| --- | --- |
| `wayback.home.how2`, `wayback.active.gapHint` | нативной фоновой службы нет **или её ещё не спросили** |
| `wayback.home.how2Background`, `wayback.active.gapHintBackground` | служба `WayBackTrack` найдена |

Признак берётся у `foregroundService()` — того же места, которым пользуется сама
запись, так что лишнего разговора с мостом не будет. Ответ асинхронный, поэтому
начальное состояние — нейтральное: мигнуть обещанием и забрать его назад хуже,
чем сказать меньше. Правило выбора живёт в `src/lib/wayback/recordingCopy.ts`
(без импортов, проверяется в Node), хук — `src/lib/wayback/useRecordingCopy.ts`,
проверка — `fastlane/.recording-copy-check.mjs`.

Тексты магазинов (`fastlane/metadata/**`) остаются нейтральными: там выбирать
не из чего, листинг читают владельцы любых версий.
...
5. `TrackRecorder` пишет точки всё время похода, в том числе со свёрнутым
   ...
   внеочередной замер. Промежутки показываются на карте **пунктиром**, и его
   легенда зависит от сборки: со своей фоновой службой это отсутствие спутников,
   без неё — уход в фон (см. «Легенда пунктира и обещание про фон» в 3.12).
...
- **Обещание про запись с погашенным экраном даётся не всем.** `how2` и
  `gapHint` существуют в двух вариантах, сильный выбирается только там, где
  нативная фоновая служба действительно есть. Заменять нейтральную строку
  сильной «насовсем» нельзя: тот же веб приезжает в старые сборки из магазинов.
